...
import 'package:booklibrary/widgets/dialog_pdf_password.dart';
...
/// ```
@immutable
class MySfPdfViewer extends StatefulWidget {
...
class SfPdfViewerState extends State<MySfPdfViewer>
  ...
  @override
  void didUpdateWidget(MySfPdfViewer oldWidget) {
  ...
  _showPasswordDialogClone() {
    Get.dialog(DialogPdfPassWord(
      callback: (pass) {
        Get.back();
        handleDecryptedPass(pass);
      },
    ));
  }

  handleDecryptedPass(String pass) {
    AppFunc.setTimeout(() {
      try {
        _decryptedProtectedDocument(_pdfBytes, pass);
      } catch (e) {
        showSnackBarAction(message: 'Invalid Password Error');
        _showPasswordDialogClone();
      }
    }, 300);
  }
  ...
  void _decryptedProtectedDocument(Uint8List pdfBytes, String? password) {
    ...
    bytes.then((value) => _decryptedBytes = Uint8List.fromList(value));
    _isEncrypted = true;
